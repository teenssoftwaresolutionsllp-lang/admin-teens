      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 sm:p-7 rounded-2xl border border-slate-200/90 shadow-sm">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-xl font-bold text-slate-900 tracking-tight">Projects, Shifts & Country Calendars</h2>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {projects.map((proj) => {
          ...
          return (
            <div
              key={proj.id}
              className="bg-white rounded-2xl border border-slate-200/90 p-5 sm:p-6 shadow-sm hover:shadow-md transition-all flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-purple-50 text-purple-700 border border-purple-200 uppercase tracking-wider">
                    {proj.client_country}
                  </span>
                ...
                <h3 className="text-base font-bold text-slate-900 mt-3 tracking-tight">{proj.name}</h3>
              ...
              <div className="mt-5 pt-3.5 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                ...
                <span className="text-[11px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">Active</span>
              </div>
            </div>
          );
        })}
        ...
        <div className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <span className="text-xs font-bold text-slate-700">
              {activeCalendar?.name} &bull; Timezone: <span className="font-mono">{activeCalendar?.timezone}</span>
            </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
            {activeCalendar?.holidays?.map((h) => (
              <div
                key={h.id}
                className="p-3.5 bg-slate-50/80 rounded-xl border border-slate-200/80 flex items-center justify-between text-xs hover:border-slate-300 transition-colors"
              >
                <div>
                  <span className="font-bold text-slate-900 block">{h.title}</span>
                  <span className="text-[11px] text-slate-500 font-mono mt-0.5 block">{h.holiday_date}</span>
                </div>
                <span className="text-[10px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-200/60 px-2 py-0.5 rounded-full uppercase tracking-wider">
                  Holiday
                </span>
              </div>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-7 shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between pb-3.5 border-b border-slate-200">
              <h3 className="text-base font-bold text-slate-900 tracking-tight">Create New Client Project</h3>
            ...
            <form onSubmit={handleCreateProject} className="mt-4 space-y-3.5 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:outline-hidden"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-bold text-slate-700 block mb-1">Client Country</label>
                  <select
                ...
                <div>
                  <label className="font-bold text-slate-700 block mb-1">Holiday Calendar</label>
                  <select
              ...
              <div className="flex items-center justify-end gap-2.5 pt-3.5 border-t border-slate-200">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-sm transition-all hover:shadow"
                >
                  Create Project
                </button>
              </div>
